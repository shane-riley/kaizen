...
# Heatmap plot
df %>% ggplot(mapping = aes(x = x1, y = x2, color = y)) +
  geom_point() +
  ggtitle("Heatmap Plot")

```

There are certainly trends buried in the data. Looking at the x2 plot shows a curve that appears somewhat quadratic, but the heatmap plot shows the y-values peaking in the middle of a cloud of different x1 / x2 combinations. Based on these observations, it seems evident that x1 and x2 both have an influence on the output variable y.
...
mod01 <- df %>% lm(y ~ (x1 + x2), data=.)
coefplot(mod01)
mod01
```
According to the plot, none of the three features are statistically significant,
though the coefficient on `x1` is nearly always positive. 
...
```{r, solution_01c}
### add more code chunks if you like
mod02 <- df %>% lm(y ~ (x1*x2), data=.)
coefplot(mod02)
mod02
```

Including the intercept, there are four features in the model. In this model, two
of the features are statistically significant, both the interaction coefficient
and the coefficient on `x1`, which was barely not significant when trained without
the interaction.
...
```{r, solution_01d}
### add more code chunks if you like
mod03 <- df %>% lm(y ~ (x1 + I(x1^2)) * (x2 + I(x2^2)), data=.)
coefplot(mod03)
mod03
```

In this model, only the intercept and the `x2` quadratic term coefficient are
...
```{r, solution_01e}
### add more code chunks if you like
mod04 <- df %>% lm(y ~ (x1 + I(x1^2) + I(x1^3) + I(x1^4))*(x2 + I(x2^2) + I(x2^3) + I(x2^4)), data=.)
coefplot(mod04)
mod04
```

In this model, only the intercept is statistically significant out of the
25 features.
...
```{r, solution_01f}
### add more code chunks if you like
train_basis <- splines::ns(df$x1, 12)
knots_use_basis <- as.vector(attributes(train_basis)$knots)
mod05 <- df %>% lm(y ~ splines::ns(x1, knots=knots_use_basis)*(x2 + I(x2^2)), data=.)
coefplot(mod05)
mod05
```
In this model, none of the features are statistically significant.
...
train_basis <- splines::ns(df$x1, 12)
knots_use_basis <- as.vector(attributes(train_basis)$knots)
mod06 <- df %>% lm(y ~ splines::ns(x1, knots=knots_use_basis)*(x2 + I(x2^2) + I(x2^3) + I(x2^4)), data=.)
coefplot(mod06)
mod06
```
In this model, none of the features are statistically significant.
...
### add more code chunks if you like
perf <- bind_rows(
  broom::glance(mod01) %>% mutate(id = 1),
  broom::glance(mod02) %>% mutate(id = 2),
  broom::glance(mod03) %>% mutate(id = 3),
  broom::glance(mod04) %>% mutate(id = 4),
  broom::glance(mod05) %>% mutate(id = 5),
  broom::glance(mod06) %>% mutate(id = 6)
)
perf %>% slice_max(order_by=r.squared)

perf %>% slice_max(order_by=AIC)

perf %>% slice_max(order_by=BIC)

perf %>% slice_min(order_by=AIC)

perf %>% slice_min(order_by=BIC)
```

Best model -- R-squared: 6
Worst model -- AIC: 1
Worst model -- BIC: 6
Best model -- AIC: 3
Best model -- BIC: 3
...
The predictions are more-or-less consistent with the AIC and BIC rankings. The worst
model according to the BIC is model 6, which from the plot clearly appears
to be overfitting the data. The AIC chooses model 1 as the worst, however. This model
is underfitting the data. The best model according to both criteria is model 3,
which seems to be a reasonable balance of complexity and generalization.
...
X03 <- df %>% model.matrix(y ~ (x1 + I(x1^2)) * (x2 + I(x2^2)), data=.)
...
X06 <- df %>% model.matrix(y ~ splines::ns(x1, df = 12)*(x2 + I(x2^2) + I(x2^3) + I(x2^4)), data=.)
...
mode = laplace_03_weak$mode[1:(length(laplace_03_weak$mode)-1)]
sd = sqrt(diag(laplace_03_weak$var_matrix))[1:(length(diag(laplace_03_weak$var_matrix))-1)]
viz_post_coefs(mode, sd, colnames(X03))
```


```{r, solution_03d_b}
### make the posterior coefficient visualization for model 6
mode = laplace_06_weak$mode[1:(length(laplace_06_weak$mode)-1)]
sd = sqrt(diag(laplace_06_weak$var_matrix))[1:(length(diag(laplace_06_weak$var_matrix))-1)]
viz_post_coefs(mode, sd, colnames(X06))
```
...
```{r, solution_03e}
### add more code chunks if you like
bayes_3_over_6_weak <- exp(laplace_03_weak$log_evidence) / exp(laplace_06_weak$log_evidence)
bayes_3_over_6_weak

```

Since the Bayes factor is greater than one, model 3 is chosen as a more plausible model. This is consistent with expectations.
...
```{r, solution_03g_a}
### add more code chunks if you like
mode = laplace_03_strong$mode[1:(length(laplace_03_strong$mode)-1)]
sd = sqrt(diag(laplace_03_strong$var_matrix))[1:(length(diag(laplace_03_strong$var_matrix))-1)]
viz_post_coefs(mode, sd, colnames(X03))
```

```{r, solution_03g_b}
### add more code chunks if you like
mode = laplace_06_strong$mode[1:(length(laplace_06_strong$mode)-1)]
sd = sqrt(diag(laplace_06_strong$var_matrix))[1:(length(diag(laplace_06_strong$var_matrix))-1)]
viz_post_coefs(mode, sd, colnames(X06))
```
...
```{r, solution_03i_a}
### add more code chunks if you like
mode = laplace_03_very_strong$mode[1:(length(laplace_03_very_strong$mode)-1)]
sd = sqrt(diag(laplace_03_very_strong$var_matrix))[1:(length(diag(laplace_03_very_strong$var_matrix))-1)]
viz_post_coefs(mode, sd, colnames(X03))
```

```{r, solution_03i_b}
### add more code chunks if you like
mode = laplace_06_very_strong$mode[1:(length(laplace_06_very_strong$mode)-1)]
sd = sqrt(diag(laplace_06_very_strong$var_matrix))[1:(length(diag(laplace_06_very_strong$var_matrix))-1)]
viz_post_coefs(mode, sd, colnames(X06))
```
...
Given the same number of training points in each analysis, the coefficient prior standard deviation affects both the mean coefficient values chosen, as well as their uncertainties. In all of the models, the uncertainties on the coefficients decrease when a stronger prior is used. Additionally, the magnitudes of the coefficients decrease with a stronger prior, as it is being declared ever more unlikely that high magnitude coefficients exist. This difference is most pronounced in model 6, since with a weak prior the coefficients chosen are large.
...
```{r, solution_03k}
### add more code chunks if you like
bayes_3_over_6_strong <- exp(laplace_03_strong$log_evidence) / exp(laplace_06_strong$log_evidence)
bayes_3_over_6_strong

bayes_3_over_6_very_strong <- exp(laplace_03_very_strong$log_evidence) / exp(laplace_06_very_strong$log_evidence)
bayes_3_over_6_very_strong
```
The strong prior agrees with the weak prior in choosing model 3 as being more plausible. The very strong prior, on the other hand, selects model 6 as being more plausible.
...
X03_test <- model.matrix(pred ~ (x1 + I(x1^2)) * (x2 + I(x2^2)), data=pred_lm_03)
...
```{r, solution_04e}
### add as many code chunks as you'd like
X06_test <- model.matrix(pred ~ splines::ns(x1, knots=make_splines_training_knots(12, df, "x1"))*(x2 + I(x2^2) + I(x2^3) + I(x2^4)), data=pred_lm_06)
...
As the prior standard deviation decreases, the posterior predictions (both mean and prediction) are pulled closer to 0. With a high prior standard deviation, the uncertainty on the posterior mean is large, whereas the prediction interval dominates when the standard deviation is small.
...
X03_glmnet <- df %>% model.matrix(y ~ (x1 + I(x1^2)) * (x2 + I(x2^2)) - 1, data=.)
X06_glmnet <- df %>% model.matrix(y ~ splines::ns(x1, df = 12)*(x2 + I(x2^2) + I(x2^3) + I(x2^4)), data=.)
```
...
After tuning, only two coefficients exist in each model.
...
Only the intercept and the quadratic `x2` term exist after tuning. The coefficient
values, however, are different across the two models.
