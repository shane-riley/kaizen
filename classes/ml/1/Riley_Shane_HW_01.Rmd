...
If you need help with understanding the R syntax please see the [R4DS book](https://r4ds.had.co.nz/) and/or the R tutorial videos and demos available on the [Canvas site](https://canvas.pitt.edu/courses/132812/pages/introduction-to-r-tutorials).  
...
The median diamond price increases with as the color quality decreases (as per the order of the dataset)
...
The figure from Problem 2b) shows a negative correlation between the median diamond price and diamond color (per the ordered hierarchy for color defined in the dataset). The figure from Problem 2f), however, shows the opposite trend. The second figure is only looking at a subset of the data (ideal cut and IF clarity), whereas the first figure is including all cuts and clarities in its medians. Additionally, the first figure includes all carats, but the second figure splits the boxplots into below-median and above-median sections. These differences are responsible for the discovery of different trends seen in the figures. Since the second figure isolates some of the other variables, it identifies the correct trend.
...
The negative relationship between displacement and efficiency is statistically significant, as the coefficient's uncertainty range is always negative.
...
All of the variables are statistically significant, and there are 3 coefficients.
