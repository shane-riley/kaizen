...
mu_0 <- 125
tau_0 <- 5
sprintf("25th Quantile value: %f",qnorm(0.25, mean = mu_0, sd = tau_0));
sprintf("75th Quantile value: %f",qnorm(0.75, mean = mu_0, sd = tau_0));
```

The interval between the 25th and 75th quantiles contains half of the probability mass.
...
```{r, normal_lowperformance}
sprintf("Probability: %f",pnorm(117, mean = mu_0, sd = tau_0, lower.tail = TRUE));
```

The prior probability that the mean performance is less than 117 units is 55 percent.
...
N = \sigma^2 \left(\frac{1}{\tau_{N}^2} - \frac{1}{\tau_{0}^2} \right)
$$

```{r, num_samples}
sigma <- 5
num_samples <- function(tau_n) ceiling(sigma^2 * ((1 / tau_n^2) - (1 / tau_0^2)))

sprintf("Posterior SD = 2 => N = %d", num_samples(2));
sprintf("Posterior SD = 0.5 => N = %d", num_samples(0.5));
...
$$
\lim_{\tau_0\to\infty} \sigma^2 \left(\frac{1}{\tau_{N}^2} - \frac{1}{\tau_{0}^2} \right) = \frac{\sigma^2}{\tau_{N}^2}
$$
```{r, num_samples_diffuse}
sigma <- 15
num_samples_diffuse <- function(tau_n) ceiling(sigma^2 * (1 / tau_n^2))
sprintf("Posterior SD = 3 => N = %d", num_samples_diffuse(3));
```

Given $\sigma = 15$ and an infinitely-vague prior, it will take 25 samples to reach $\tau_{N} = 3$.
...
$$
\log \left( \prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \times \mathrm{normal}\left( \mu \mid \mu_0, \tau_0\right) \right) \\
\to \sum_{n=1}^{N} \left( \log \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right)\right)\right) + \log \left( \mathrm{normal} \left( \mu \mid \mu_0, \tau_0\right)\right) \\
\to \sum_{n=1}^{N} \left( \log \left( e^{\frac{-1}{2 \sigma^2} \left(x_n - \mu \right)^2}\right)\right)
+ \log \left( e^{\frac{-1}{2 \tau_0^2} \left(\mu - \mu_0 \right)^2}\right) \\
\to -\frac{1}{2 \sigma^2} 
\sum_{n=1}^{N} \left(x_n - \mu \right)^2
- \frac{1}{2 \tau_0^2} \left(\mu - \mu_0 \right)^2 \\
...
$$
\frac{\mathrm{d}}{\mathrm{d}\mu} \left( -\frac{1}{2 \sigma^2} 
\sum_{n=1}^{N} \left(x_n - \mu \right)^2
- \frac{1}{2 \tau_0^2} \left(\mu - \mu_0 \right)^2 \right) \\
\to -\frac{1}{2 \sigma^2} 
\sum_{n=1}^{N} \left(2 \right)\left(x_n - \mu \right)\left(-1 \right)
- \frac{1}{\tau_0^2}(\mu - \mu_0) \\
\to \frac{1}{\sigma^2}\sum_{n=1}^N \left(x_n - \mu \right) 
...
$$
\frac{1}{\sigma^2}\sum_{n=1}^N \left(x_n - \mu_{MAP} \right) 
- \frac{1}{\tau_0^2} \left(\mu_{MAP} - \mu_0 \right) = 0 \\
N \tau_0^2 \left(\bar{x} - \mu_{MAP} \right) 
- \sigma^2 \left( \mu_{MAP} - \mu_0 \right) = 0 \\
N \tau_0^2 \bar{x} + \sigma^2 \mu_0 = \mu_{MAP} \left( N \tau_0^2 + \sigma^2 \right) \\
\mu_{MAP} = \frac{N \tau_0^2 \bar{x} + \sigma^2 \mu_0}{N \tau_0^2 + \sigma^2} \\
...
This formula matches that of the mean of the posterior distribution, $\mu_N$.
Since the prior and likelihood are both Gaussian, the posterior distribution is
also a Gaussian. In Gaussians, values for mean and for mode are equal. That is
consistent with the observation that $\mu_{MAP} = \mu_N$.
...
\to \sum_{n=1}^N \left( \log \left( \frac{1}{\sigma}\right) + \log \left( \mathrm{normal}\left( x_n \mid \mu, \sigma \right) \right)\right) + \log \left( \mathrm{Exp \left( \sigma \mid \lambda\right)} \right) \\
\to - N \log \left( \sigma \right) - \frac{1}{2 \sigma^2} \sum_{n=1}^N \left( x - \mu\right)^2 - \lambda \sigma
$$
...
$$
- N \log \left( \sigma \right) - \frac{2N}{\sigma^2} \frac{1}{N} \sum_{n=1}^{N} \left( x_n - \mu \right)^2 - \lambda \sigma \\
\to - N \log \left( \sigma \right) -\frac{2N}{\sigma^2} v - \lambda \sigma
$$

$v$ can be considered "sufficient" in this context because it contains all of the 
empirical data in the expression within itself. In a sense, it "encapsulates" the
information that comes from experiment and is necessary to produce the posterior
distribution. After making use of $v$, the expression does not depend on the observations
outside of this factor.
...
{
  (-N * log(sigma)) - (2 * N * v / sigma^2) - (lambda * sigma)
}
```
...
The log posterior in this context does have one mode, but is not a parabola. It is
not expected to be a parabola, since the posterior is not a Gaussian
distribution.
...
The inverse link function is the inverse of the logarthm:
...
$$
\frac{\mathrm{d}}{\mathrm{d}\varphi} g^{-1}(\varphi) = \frac{\mathrm{d}}{\mathrm{d}\varphi} \exp \left(\varphi\right) = \exp \left( \varphi \right)
$$

Finally, the logarithm is applied:

$$
\log \left( \left|\frac{\mathrm{d}}{\mathrm{d}\varphi} g^{-1}(\varphi)\right| \right) = \log \left( \exp \left( \varphi \right) \right) = \varphi
$$
...
log_post_varphi_unnorm <- function(varphi, N, v, lambda)
{
  ...
  log_post <- (-N * log(sigma)) - (2 * N * v / sigma^2) - (lambda * sigma)
...
upper_bound = log(4.5)
tibble::tibble(
  varphi = seq(lower_bound, upper_bound, length.out = 101)
) %>% 
...
On the left-hand side of the maximum, the curve does look more parabolic. The
right side, however, does not.
...
upper_bound <- (9 * pi / 8)
tibble::tibble(
  x = seq(lower_bound, upper_bound, length.out = 1001)
) %>% 
...
upper_bound <- 2
tibble::tibble(
  x = seq(lower_bound, upper_bound, length.out = 501)
) %>% 
...
The posterior mode is about the same between the two guesses.
