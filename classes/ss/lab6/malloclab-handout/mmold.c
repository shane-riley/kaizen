...
void mm_free(void *bp)
{
      ...
      // Precondition: bp is the beginning of a block
      block_t *block = payload_to_header(bp);
      size_t block_size = get_size(block);

      // Adjust header
      write_header(block, block_size, 0);

      // Adjust footer
      write_footer(block, block_size, 0);
...
static void insert_block(block_t *free_block)
{
      // Head is always the front of the list (no prev)
      // The inserted block becomes the new head
...
static void remove_block(block_t *free_block)
{
      // Stitch previous and next together

      // Get next and prev blocks
      ...
      if (free_block == free_list_head) free_list_head = next_block;
}
...
{
      // 4 cases
      if (find_prev(block) == block) return block;
      bool p_a = get_alloc(find_prev(block));
      bool n_a = get_alloc(find_next(block));

      // Assume block in free list
      ...
      else if (p_a)
      {
            ...
            // Remove the eaten block from the free list
            remove_block(find_next(block));

            // Write the new size
            size_t new_size = get_size(block) + get_size(find_next(block));
      ...
      else
      {
            ...
            size_t new_size = get_size(block) + get_size(b1) + get_size(b2);
            block = find_prev(block);
            write_header(block, new_size, false);
            write_footer(block, new_size, false);
      }
...
static void split_block(block_t *block, size_t asize)
{
      ...
      // Assume block not in free list
      // Alloc and full size in header and footer

      if (block_size - asize >= min_block_size)
      {
            // We can split the block into asize and block_size-asize
            ...
            insert_block(block);
            write_header(block, block_size - asize, false);
            write_footer(block, block_size -  asize, false);
      }
}
...
static block_t *extend_heap(size_t size)
{
      ...
      // Epilogue must move to end

      // pointer points to header of block (steal old epilogue location)
      ...
      word_t *new_ep = (word_t *) find_next(block);

      // Write epilogue
      *new_ep = pack(0, true);


      heap_end = (block_t *)  new_ep;
