...
int intSize() {
  ...
  // The size of an int is unknown, but the size of a char as one. 
  // We get the number of bytes between two pointers by casting to char* and then subtracting

  intPtr2 = intArray;
  intPtr2++;  // Move forward one int

  intPtr1 = intArray;  // This one is one int behind

  return (int) ((char *) intPtr2 - (char *) intPtr1);
}
...
int doubleSize() {
  ...
  // See comments for intSize
  doubPtr2 = doubArray;
  doubPtr2++;
  doubPtr1 = doubArray;

  return (int) ((char *) doubPtr2 - (char *) doubPtr1);
}
...
int pointerSize() {
  ...
  return (int) ( (char *) ptrPtr2 - (char *) ptrPtr1);
}
...
int withinSameBlock(int * ptr1, int * ptr2) {
  ...
  // If the block is the same, all but the last six bits must be the same  
  long longPtr1 = (long) ptr1;
  long longPtr2 = (long) ptr2;

  return ((longPtr1 >> 6) == (longPtr2 >> 6));
}
...
int withinArray(int * intArray, int size, int * ptr) {
  ...
  int byteDist = (int) ((char *) ptr - (char *) intArray);
  int totalBytes = size * intSize();

  // If byteDist is negative, fails
  int negativeDist = (~(byteDist >> 31) == 0);

  // If difference - 1 is negative, fails
  int negativeDiff = (~((totalBytes - byteDist - 1) >> 31) == 0);

  return !(negativeDiff + negativeDist);
}
...
int endianExperiment(int * ptr) {
  ...
  // The integer representation is 0x0006DBA9
...
void selectionSort(int arr[], int arrLength) {
  int i, j, min_index;
  // Your code here
  // i is the first unsorted index
  // j is for finding min index
  // min_index is the index of the minimum value in the unsorted section
